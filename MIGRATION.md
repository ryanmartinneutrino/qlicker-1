...
| **Testing** | Vitest 4 + Playwright | Unit tests (367 server + 89 client) plus E2E flows |
...
- ✅ Course-manage response-tracking polish — sessions now persist `hasResponses` plus per-question response counts, paginated session-list responses expose per-type totals for stable tab counts/control layout, professor and student course pages load the first 15 sessions immediately then hydrate the rest in the background, single-session status/metadata updates now patch the affected course-page row instead of forcing a full list rehydrate, live interactive sessions now expose the same “Review Live Session Results” action as quizzes, ended session cards rely on click-through review instead of a redundant button, and draft/upcoming review buttons only appear when tracked response data exists
...
| Admin user support | User modal includes password reset, email-login exception controls, and current/last session activity with IP addresses | This replaces ad hoc support workflows for SSO-managed accounts. |
...
| Login/session audit metadata | `lastLogin`, `lastLoginIp`, and active refresh-session IPs are persisted | Admin users can see who is currently logged in and the latest known IP for each active session. |
...
| Student response audit metadata | Each saved response records `submittedAt` and `submittedIpAddress` | Stored as normal MongoDB dates/strings so timestamps remain timezone-independent while following server time at write time. |
...
- ✅ **Sessions list pagination** — `GET /courses/:courseId/sessions` now accepts optional `page`/`limit` query params; returns `total`/`page`/`pages` plus `sessionTypeCounts` when pagination is active so course pages can render stable tab counts and list controls before background hydration completes; backward-compatible (returns all sessions when params omitted)
