...
- `Agent-04 Grading`: instructor grading workflow parity (group/category filtering + bulk assignment landed; session-selected aggregate `CourseGrades` view now landed; remaining parity checks pending).
...
- `Agent-07 Realtime/Perf`: realtime routing correctness + perf hardening (question channel sanitization/invalidation hardening, dynamic response-stats privacy refresh, attempt-scoped `/api/responses` hot-path optimization, and load harness `npm run test:migration-load` landed).
...
## Next Milestone
Execute runtime verification (`MIG-040`, `MIG-041`, `MIG-042`, `MIG-033`, `MIG-043`, `MIG-044`) in Docker/CI with latest `master` and close final run-session/question-editor/group-video parity deltas.
