...
| `imports/api/sessions.js` methods | `packages/server/src/routes/sessions.ts`, session pages in client | `sessions`, `courses.sessions` | partial | Session question attach/remove/reorder workflow now landed via REST + `ManageSession`; full lifecycle smoke/e2e validation pending | Agent-02, Agent-03 | pending |
