...
This document outlines the high level goals for the migration and should be referred to during the migration.
...
- I want to have several tests that test various "flows". For example, the "login flow" would involve a user going to the main website, being shown the /login endpoint, then clicking to create and account, then loggin in and updating their profile pic and password (for example, or some other interaction). Then logging back out and back in. These tests should be done from the frontend (which will also test the backend). On a page, for example, one should simulate that every button that is clicked as the expected behaviour. It would be costly to run these at each PR during the migration, these would only run at big milestones in the migration. Develop such "flow tests" and describe them in the documentation in plain english. 
